...
- **What it provisions** (into the resource group you choose): a Log Analytics workspace, a Container Apps managed environment, and the Container App. **Max replicas is always 1** (the in-memory design can't be split across replicas). **Minimum replicas defaults to 1 (always-on, recommended)**; you can pick **0** to scale to zero when idle for trial use — cheaper, but each idle period loses all in-memory state (history, generated media) and the next message cold-starts. `App__PublicBaseUrl` is set automatically to the app's own HTTPS URL, so there's no two-step FQDN step.
- **After it finishes:** the webhook URL is on the deployment's **Outputs** tab (Resource group → Deployments → your deployment → Outputs, as `lineWebhookUrl`) — it is **not** shown on the completion screen. You can also build it yourself from the Container App's **Application Url** + `/webhook`. Register it in LINE (continue from [step 5](#5-point-line-at-the-webhook)), and verify with `curl https://<fqdn>/health` → `{"status":"ok"}`.

> **Cost:** with Minimum replicas = 1 the app runs 24/7 (no scale-to-zero), so expect a small always-on charge; set it to 0 to avoid idle cost at the price of cold starts and lost state. Tear everything down by deleting the resource group (portal, or `az group delete --name <rg>`).
