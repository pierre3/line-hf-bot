...
| `HuggingFace__VisionModel` / `HuggingFace__VisionEndpoint` | vision Q&A for sent photos, via a vision chat model on the OpenAI-compatible endpoint (default `Qwen/Qwen2.5-VL-7B-Instruct`). Uses chat-level HF credits (not fal). Availability is provider-dependent; the "Ask" button fails with a generic error if your token can't serve it |
